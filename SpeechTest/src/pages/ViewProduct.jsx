...
const handleRenameCategory = (oldName) => {
  if (newCategoryName.trim() === '') {
    return toast.info('Category name cannot be empty');
  }
  axios.put(`${import.meta.env.VITE_API_URL}/renamecategory`, {
    ...
    .then(() => {
      ...
      axios.get(`${import.meta.env.VITE_API_URL}/categorieswithcount`)
        .then(res => setCategories(res.data))
        .catch(err => toast.info("Error fetching categories:"));
    })
    .catch((err) =>{ setNewCategoryName("");toast.info("Error renaming category:")});
};
...
const handleDeleteSelected = () => {
  ...
  const [visible, setVisible] = useState(false);
    ...
    const accept = () => {
      ...
      .then((msg) => {
        toastt.current.show({ severity: 'success', summary: 'Success', detail: 'Category and Product Deleted Successfully', life: 3000 });
        axios.get(`${import.meta.env.VITE_API_URL}/categorieswithcount`)
        .then(res => setCategories(res.data))
        .catch(err =>{ toast.error("Error fetching categories:")});
      })
      .catch(err =>{ toastt.error("Error deleting products")});
        
    }
    const reject = () => {
        toastt.current.show({ severity: 'warn', summary: 'Rejected', detail: 'You have rejected', life: 3000 });
    }
