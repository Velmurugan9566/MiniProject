...
const toggleAside = () => {
  ...
  useEffect(() => {
    ...
    setnewSupp("");
    setChanges(false);
    //console.log('Fetch cate and supp');
  ...
  const handleSupplier = (e) => {
    ...
    if (!newSuppid) {
      ...
      toast.warning("enter the valid name")
    }else if(!newSuppEmail || !/.+\@.+\..+/.test(newSuppEmail)){
        toast.warning("Enter a valid Email address..")
    }else if(!newSuppPhone || !/^[0-9]{10}$/.test(newSuppPhone)){
      toast.warning("Enter a valid phone number.");
    }
    else {
      axios.post(`${import.meta.env.VITE_API_URL}/Addsupp`, { suppname: newSupp.toLowerCase(), suppid: newSuppid,suppEmail:newSuppEmail,suppPhone:newSuppPhone })
        .then(res => {
          console.log(res)
          if (res.data.status == 1) {
      ...
      {showSupplierPopup && (
        <div className="popup active">
          ...
          <form>
            ...
            <input type="text" value={newSupp} onChange={(e) => setnewSupp(e.target.value.toLowerCase())} /><br/>
            <label>Supplier Email:</label>
            <input type="text" value={newSuppEmail} onChange={(e) => setNewSuppEmail(e.target.value.toLowerCase())} /><br/>
            <label>Supplier PhoneNo:</label>
            <input type="text" inputMode='numeric' value={newSuppPhone} onChange={(e) => setNewSuppPhone(e.target.value.toLowerCase())} />
           <br/><center> <button type="button" onClick={(e) => { setShowSupplierPopup(false); handleSupplier(e) }}>Add</button></center>
          </form>
